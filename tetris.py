...
def dropPiece():
	...
	while movePossible:
		...
		if movePossible: canvas.data.Moved += 1
	canvas.data.spacePressed = False
	timerFired()
...
def drawGame():
	canvas.create_rectangle(0,0,canvas.data.cols*20 + 140,canvas.data.rows*20 + 40, fill = "black")
	drawBoard()
	drawShadow()
	drawFallingPiece()
...
def drawShadow():
	if not canvas.data.isGameOver:
		...
		while shadowPieceIsLegal(): canvas.data.shadowPieceRow += 1
		canvas.data.shadowPieceRow -= 1
		#draw it
		for row in xrange(len(canvas.data.shadowPiece)):
			for col in xrange(len(canvas.data.shadowPiece[0])):
				if canvas.data.shadowPiece[row][col]:
...
class RoomDialog(tkSimpleDialog.Dialog):
	def body(self, master):
		Label(master, text="What is the room name?").grid(row=0)
